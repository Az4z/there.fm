...
function endCall(silent){
  ...
  localAnalyser=null; rawAnalyser=null; _gateBypassed=false; stopMicWatchdog(); stopReceiveWatchdog();
...
function createCallPeerConnection(uid){
  ...
  const entry={pc,audioEl:null,analyser:null,pendingCandidates:[],
               ...
               lastBytes:0, stalledFor:0, iceTries:0, discTimer:null};

  /* Garante um canal de áudio BIDIRECIONAL desde o início. Antes, se por
     qualquer motivo a trilha local ainda não existisse na hora de criar a
     conexão, nascia uma conexão só de recepção — e a pessoa te ouvia mas
     você não ouvia ela (ou o contrário). É uma das causas do áudio "de um
     lado só" que você relatou. */
  try{ pc.addTransceiver('audio',{direction:'sendrecv'}); }catch(e){}
  if(localSentStream){
    localSentStream.getTracks().forEach(t=>{
      const sender=pc.getSenders().find(s=>!s.track && s.track!==t);
      if(sender) sender.replaceTrack(t); else pc.addTrack(t,localSentStream);
    });
  }
  ...
  /* Renegocia sozinho quando o navegador pede (troca de trilha, reinício de ICE).
     Sem isso, qualquer mudança no meio da chamada podia deixar o áudio mudo até
     alguém sair e entrar de novo. */
  pc.onnegotiationneeded=async()=>{
    try{
      ...
      broadcast({type:'CALL_OFFER',uid:U.id,to:uid,sdp:pc.localDescription});
  ...
  pc.ontrack=e=>{
    ...
    if(!audioEl){
      ...
      audioEl.style.display='none'; audioEl.dataset.uid=uid;
    ...
    try{
      const srcNode=callAudioCtx.createMediaStreamSource(remoteStream);
      const an=callAudioCtx.createAnalyser(); an.fftSize=512; srcNode.connect(an); entry.analyser=an;
    }catch(err){}
    updateCallPeerState(uid,'ok');
  };

  /* Recuperação por estado da conexão:
     - "disconnected" costuma ser passageiro (rede oscilando): espera 4s e, se
       não voltar sozinho, reinicia a negociação de rota.
     - "failed" não volta sozinho: reinicia na hora.
     Antes só havia tratamento para "failed", então quedas passageiras podiam
     virar silêncio permanente sem ninguém perceber. */
  pc.oniceconnectionstatechange=()=>{
    const st=pc.iceConnectionState;
    updateCallPeerState(uid,st);
    clearTimeout(entry.discTimer);
    if(st==='disconnected'){
      entry.discTimer=setTimeout(()=>{
        if(pc.iceConnectionState==='disconnected') restartPeerIce(uid);
      },4000);
    }
    if(st==='failed') restartPeerIce(uid);
    if(st==='connected'||st==='completed'){ entry.iceTries=0; entry.stalledFor=0; }
  };
  pc.onconnectionstatechange=()=>{
    updateCallPeerState(uid,pc.connectionState);
    if(pc.connectionState==='failed') restartPeerIce(uid);
  };
  return entry;
}
/* Alguns navegadores bloqueiam áudio até haver interação. Se o play falhar,
   registramos para tentar de novo no próximo toque em qualquer lugar da tela. */
function playRemoteAudio(el){
  const p=el.play();
  if(p&&p.catch) p.catch(()=>{
    _pendingAudioPlay=true;
    document.addEventListener('pointerdown',resumeAllRemoteAudio,{once:true});
  });
}
let _pendingAudioPlay=false;
function resumeAllRemoteAudio(){
  _pendingAudioPlay=false;
  Object.values(callPeers).forEach(cp=>{
    if(cp.audioEl){ const p=cp.audioEl.play(); if(p&&p.catch)p.catch(()=>{}); }
  });
  if(callAudioCtx&&callAudioCtx.state==='suspended') callAudioCtx.resume().catch(()=>{});
}
/* Reinicia a busca por uma rota de conexão sem derrubar a chamada. */
function restartPeerIce(uid){
  const entry=callPeers[uid]; if(!entry) return;
  if(entry.iceTries>=4){                 // desistiu de reparar: refaz do zero
    console.warn('Reconstruindo conexão de voz com',uid);
    const info=callParticipants[uid];
    handleCallPeerGone(uid);
    if(callActive){ callConnectTo(uid,info); }
    return;
  }
  entry.iceTries++;
  try{
    if(entry.pc.restartIce) entry.pc.restartIce();
    else if(U.id<uid) callMakeOffer(uid,true);   // navegador antigo: oferta com reinício
  }catch(e){ console.error('restartPeerIce',e); }
}
...
/* NEGOCIAÇÃO COM REGRA DE CORTESIA ("perfect negotiation").
   Antes, se os dois lados enviassem uma oferta quase ao mesmo tempo — o que
   acontece bastante ao entrar junto na chamada ou depois de uma queda de rede —
   um dos lados recebia oferta num momento inválido, dava exceção, e a conexão
   ficava pela metade: um ouvia e o outro não, sem nenhum erro visível.
   A regra: quem tem o uid maior é o "educado" e cede em caso de conflito,
   desfazendo a própria oferta para aceitar a do outro. Assim a negociação
   sempre converge, não importa quem falou primeiro. */
async function handleCallOffer(uid,sdp){
  ...
  const entry=callPeers[uid], pc=entry.pc;
  try{
    const offerColide = entry.makingOffer || pc.signalingState!=='stable';
    entry.ignoreOffer = !entry.polite && offerColide;
    if(entry.ignoreOffer) return;            // eu não sou o educado: mantenho a minha oferta
    if(offerColide){                          // eu sou o educado: desfaço a minha e aceito a dele
      await Promise.all([
    ...
    (entry.pendingCandidates||[]).forEach(c=>pc.addIceCandidate(c).catch(()=>{}));
    entry.pendingCandidates=[];
    const answer=await pc.createAnswer();
    answer.sdp=tuneOpusSdp(answer.sdp);
    await pc.setLocalDescription(answer);
    broadcast({type:'CALL_ANSWER',uid:U.id,to:uid,sdp:pc.localDescription});
