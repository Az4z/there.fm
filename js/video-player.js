...
function initHtml5(uid,src,cont){
  ...
  vidEl.addEventListener('error',()=>{
    ...
    if(holder && !holder.querySelector('.ifr-fallback')){
      ...
      d.innerHTML='<span>O site bloqueou a reprodução deste arquivo fora do domínio dele.'
        +'<br>Use a opção <b>Buscar na página</b> e escolha um resultado do tipo <b>Player</b>.</span>'
        +'<a href="'+src+'" target="_blank" rel="noopener">Abrir o arquivo ↗</a>';
      holder.style.position='relative';
      holder.appendChild(d);
    }
    toast('O site bloqueou este arquivo fora do domínio dele','err');
  });
  ytPlrs[uid]={
