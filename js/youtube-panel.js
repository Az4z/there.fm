...
/* ══════════════════════════════════════════════════════════════════
   VIGIA DE RECEPÇÃO — ataca direto o "não escuto a pessoa".
   O estado da conexão pode dizer "conectado" enquanto NENHUM áudio chega:
   a rota se degradou, o TURN caiu, o celular trocou de rede. Visualmente
   está tudo certo e ninguém entende por que o outro sumiu.
   Aqui perguntamos ao navegador quantos bytes de áudio realmente chegaram
   de cada participante. Se o número parar de crescer por ~8 segundos com a
   pessoa ativa, a conexão está morta na prática e refazemos a rota sozinhos. */
let _rxWatch=null;
function startReceiveWatchdog(){
  stopReceiveWatchdog();
  _rxWatch=setInterval(async ()=>{
    ...
    for(const uid of Object.keys(callPeers)){
      ...
      try{
        ...
        if(bytes>(entry.lastBytes||0)){
          entry.lastBytes=bytes; entry.stalledFor=0;
...
function stopReceiveWatchdog(){ if(_rxWatch){ clearInterval(_rxWatch); _rxWatch=null; } }
