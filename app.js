...
async function openProfile(uid){
  ...
  $('pubName').innerHTML=`<span class="pname-row">`
    +`<span class="pname-txt">${escapeHtml(p.name||'Usuário')}</span>`
    +(isOwnerUid(uid)?ownerBadgeHTML():'')
    +`<span class="ptags" id="pubTagsInline"></span>`
    +`</span>`;
...
async function renderProfileTags(uid){
  _tagCtxUid=uid;
  const box=$('pubTagsInline')||$('pubTags'); const admin=$('pubTagAdmin');
