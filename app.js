...
async function openProfile(uid){
  ...
  $('pubName').innerHTML=(p.name||'Usuário')+(isOwnerUid(uid)?ownerBadgeHTML():''); $('pubUname').textContent=p.username?('@'+p.username):''; $('pubBio').textContent=p.bio||'';
...
function ownerBadgeHTML(){
  return `<span class="owner-badge" title="Dono da plataforma"><img src="${OWNER_BADGE_GIF}" alt=""><b>Owner</b></span>`;
}
/* Linha flex com nome + badge — evita que a reticência (...) de nomes compridos corte o badge,
   já que o badge fica fora da caixa que trunca o texto, não dentro dela. */
function nameRowHTML(name,uid,nameClass){
  return `<div class="name-row"><div class="${nameClass}">${name||'Usuário'}</div>${isOwnerUid(uid)?ownerBadgeHTML():''}</div>`;
}
