...
async function openProfile(uid){
  ...
  $('pubName').innerHTML=`<span style="display:inline-flex;align-items:center;flex-wrap:wrap;row-gap:.3rem">${p.name||'Usuário'}${isOwnerUid(uid)?ownerBadgeHTML():''}</span>`; $('pubUname').textContent=p.username?('@'+p.username):''; $('pubBio').textContent=p.bio||'';
