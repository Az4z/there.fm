...
function initBoardViewportEvents(){
  ...
  outer.addEventListener('wheel',e=>{
    e.preventDefault();
    if(e.ctrlKey||e.metaKey) boardZoomAt(e.clientX,e.clientY,e.deltaY<0?1.12:1/1.12);
    else if(e.shiftKey){ boardView.x-=e.deltaY; applyBoardTransform(); }
    else { boardView.x-=e.deltaX; boardView.y-=e.deltaY; applyBoardTransform(); }
  },{passive:false});
  ...
  outer.addEventListener('pointermove',e=>{
    if(!boardPan) return;
    boardView.x=boardPan.ox+(e.clientX-boardPan.startX);
    boardView.y=boardPan.oy+(e.clientY-boardPan.startY);
    applyBoardTransform();
  });
...
function autoGrowNode(id){
  ...
  const body=el.querySelector('.board-node-body'); if(!body) return;
  const n=boardNodes[id];
  el.style.height='auto';
  const needed=el.scrollHeight;
  if(n && (!n.h || needed>n.h)){ n.h=needed; }
  scheduleEdgeRedraw();
}
...
function startBoardDrag(e,id){
  ...
  const el=document.getElementById('bn-'+id); if(!el) return;
  boardDrag={ id, startX:e.clientX, startY:e.clientY, startLeft:parseFloat(el.style.left)||0, startTop:parseFloat(el.style.top)||0 };
...
function onBoardResizeMove(e){
  ...
  const n=boardNodes[boardResize.id]; if(n){ n.w=w; n.h=h; }
  scheduleEdgeRedraw();
}
...
async function handleBoardConnectClick(id){
  ...
  boardEdges[data.id]=data; scheduleEdgeRedraw();
}
...
function edgeGeometry(edge){
  ...
  if(!a||!b) return null;
  const ea=document.getElementById('bn-'+edge.from_node), eb=document.getElementById('bn-'+edge.to_node);
  const aw=ea?ea.offsetWidth:(a.w||180), ah=ea?ea.offsetHeight:(a.h||120);
  const bw=eb?eb.offsetWidth:(b.w||180), bh=eb?eb.offsetHeight:(b.h||120);
  ...
  // ponto onde a curva encosta em cada card: interseção do retângulo na direção do controle
...
function scheduleEdgeRedraw(){
  if(_edgeRAF) return;
  _edgeRAF=requestAnimationFrame(()=>{ _edgeRAF=null; redrawBoardEdges(); });
}
...
function redrawBoardEdges(){
  ...
  ensureEdgeDefs(svg);
  Object.keys(_edgeEls).forEach(id=>{ if(!boardEdges[id]) removeEdgeEls(id); });
...
async function deleteBoardEdge(id){
  ...
  delete boardEdges[id]; removeEdgeEls(id); scheduleEdgeRedraw();
}
