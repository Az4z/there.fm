...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  // Intro curta e enxuta: ~1,4s no total (antes eram ~8s somando as duas telas).
  // Removida assim que sai de cena para não deixar uma camada de tela cheia no DOM.
  setTimeout(()=>{
...
function openSettings(){
  ...
  initProfilePreview();
  if($('profColor')) $('profColor').value=normHex(U.color)||'#c45c5c';
  if($('profColorHex')) $('profColorHex').value=normHex(U.color)||'#c45c5c';
  if($('profColorDot')) $('profColorDot').style.background=U.color||'#c45c5c';
  if($('profTintCard')) $('profTintCard').checked=!!U.card_tint;
...
/* ══════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════ */
const PV_SUGGEST=['#c45c5c','#e0a35c','#d8c47a','#7fc8a9','#6db5c9','#8ab4f5','#b79ae0','#e08fb0','#9aa4b0','#5c6b7a'];
function initProfilePreview(){
  const sw=$('pvSwatches'); if(sw&&!sw.dataset.ready){
    sw.dataset.ready='1';
    sw.innerHTML=PV_SUGGEST.map(c=>`<i data-c="${c}" style="background:${c}" title="${c}"></i>`).join('');
    sw.addEventListener('click',e=>{ const i=e.target.closest('i'); if(i) onProfColorInput(i.dataset.c); });
  }
}
function normHex(v){
  v=String(v||'').trim();
...
function onProfColorInput(v,typing){
  const hex=normHex(v);
  if(!hex){ if(typing) return; return; }   // digitando parcial: espera terminar
  U.color=hex;
  const p=$('profColor'); if(p&&p.value.toLowerCase()!==hex) p.value=hex;
  const t=$('profColorHex'); if(t&&document.activeElement!==t) t.value=hex;
  const dot=$('profColorDot'); if(dot) dot.style.background=hex;
  renderProfilePreview();
}
function renderProfilePreview(){
  const card=$('pvCard'); if(!card) return;
  const color=U.color||'#c45c5c';
  const [r,g,b]=hexToRgb(color);
  const tint=$('profTintCard')?$('profTintCard').checked:false;
  U.card_tint=tint;
  // fundo do card: neutro, ou tingido com a cor escolhida
  card.style.background = tint
    ? `linear-gradient(160deg,rgba(${r},${g},${b},.30),rgba(${r},${g},${b},.10) 55%,rgba(10,12,11,.96))`
    : 'linear-gradient(160deg,rgba(255,255,255,.045),rgba(10,12,11,.96))';
  card.style.borderColor=`rgba(${r},${g},${b},.45)`;
  card.style.boxShadow=`0 12px 34px rgba(0,0,0,.5),0 0 0 1px rgba(${r},${g},${b},.18)`;
  const glow=$('pvGlow'); if(glow) glow.style.background=`radial-gradient(circle at 22% 0%,rgba(${r},${g},${b},.35),transparent 62%)`;
  const bn=$('pvBanner');
  if(bn){
    bn.style.backgroundImage=U.banner?`url('${U.banner}')`:'none';
    bn.style.background=U.banner?`url('${U.banner}') center/cover`:`linear-gradient(100deg,rgba(${r},${g},${b},.75),rgba(${r},${g},${b},.25))`;
  }
  const av=$('pvAv'); if(av) av.innerHTML=avatarHTML(U);
  const nm=$('pvName'); if(nm) nm.textContent=($('sNm')&&$('sNm').value.trim())||U.name||'Seu nome';
...
async function saveSettings(){
  ...
  const payload={ id:user.id, name:U.name, email:U.email, username:U.username, bio:U.bio, color:U.color, card_tint:!!U.card_tint, photo:U.photo||null, banner:U.banner||null, frame:U.frame||null, frame_scale:U.frame_scale||1, frame_x:U.frame_x||0, frame_y:U.frame_y||0 };
  let { data, error }=await supa.from('profiles').upsert(payload,{onConflict:'id'}).select().maybeSingle();
  if(error && /card_tint/.test(error.message||'')){
    // banco ainda sem a coluna card_tint: salva o resto normalmente em vez de perder tudo
    const { card_tint, ...rest }=payload;
    ({ data, error }=await supa.from('profiles').upsert(rest,{onConflict:'id'}).select().maybeSingle());
    if(!error) console.warn('Coluna card_tint ausente — adicione-a para salvar o tingimento do card.');
  }
...
async function openProfile(uid){
  ...
  if(_mbox){
    _mbox.style.background = p.card_tint
      ? `linear-gradient(160deg,rgba(${_r},${_g},${_b},.26),rgba(${_r},${_g},${_b},.08) 55%,rgba(10,12,11,.98))`
      : '';
  ...
  _profileCtx={ uid, name:p.name||'Usuário' };
...
function renderNameRow(){
  ...
  el.innerHTML='<span class="pname-row">'
    +`<span class="pname-txt">${escapeHtml(_profileCtx.name)}</span>`
...
function toggleNoiseGate(){
  ...
  applyGateParams();
  if(noiseGateEnabled) _gateBypassed=false;  // reativado manualmente: volta a confiar nele
  updateGateBtnUI();
